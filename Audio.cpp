...
int InitDirectSound( HWND hwnd)
{
   ...
   PrintLog("DirectSoundEnumerate: Ok\n");
	...
	sprintf(logtt,"Device selected  : %d\n",iCurrentDriver);
	PrintLog(logtt);
	...
	if( (hres != DS_OK) || (!lpDS) )
	{
		sprintf(logtt, "DirectSoundCreate Error: %Xh\n", hres);
		PrintLog(logtt);
		return 0;
	}
	...
	else PrintLog("Set Cooperative  : Ok\n");
...
BOOL CALLBACK EnumerateSoundDevice( GUID* lpGuid, LPSTR lpstrDescription, LPSTR lpstrModule, LPVOID lpContext)
{
   ...
   CopyMemory( sdd[iTotalSoundDevices].lpGuid, lpGuid, sizeof( GUID ) );
/*
